cars = [{'make': 'Toyota', 'model': 'Corolla', 'price': 35500, 'year': 2018},
        ...
        {'make': 'Land Rover', 'price': 86000,
            'year': 2022, 'model': 'Range Rover'},
        {'make': 'Hyundai', 'price': 33100, 'year': 2019, 'model': 'Sonata'}
        ]


def sortedList(li):
    x = sorted(li, key=lambda x: x['price'])
    print(x)


def carsMadeInSameYear(li, year):
    return list(filter(lambda x: x['year'] == year, li))


print(carsMadeInSameYear(cars, 2019))
